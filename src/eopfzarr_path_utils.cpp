...
PathParser::ParsedPath PathParser::Parse(const std::string& fullPath)
{
    ...
    if (startQuote != std::string::npos)
    {
        ...
        if (endQuote != std::string::npos && endQuote > startQuote + 1)
        {
            ...
            mainPathForChecking =
                pathWithoutPrefix.substr(startQuote + 1, endQuote - startQuote - 1);
        }
    }
    ...
    if (result.isUrl || result.isVirtualPath)
    {
        ...
        if (startQuote != std::string::npos)
        {
            ...
            if (endQuote != std::string::npos && endQuote > startQuote + 1)
            {
                ...
                result.isSubdataset =
                    false;  // URLs with subdatasets are treated as single main paths
                return result;
            }
        }
...
void PathParser::NormalizeWindowsPath(std::string& path)
{
    ...
    if (!path.empty() && path[0] == '\\' && path.length() > 2 && path[1] != '\\' && path[2] == ':')
    {
        path = path.substr(1);
    }
