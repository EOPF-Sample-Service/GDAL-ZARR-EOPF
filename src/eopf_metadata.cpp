...
static void ExtractCoordinateMetadata(const CPLJSONObject &obj, GDALDataset &ds)
{
    ...
    if (stacDiscovery.IsValid())
    {
        ...
        if (properties.IsValid())
        {
            ...
            if (!epsg.empty())
            {
                CPLDebug("EOPFZARR", "Found proj:epsg in STAC properties: %s",
                         epsg.c_str());
            }
        }
    }
    ...
    if (epsg.empty())
    {
        ...
        if (!epsg.empty())
        {
            CPLDebug("EOPFZARR", "Found proj:epsg at top level: %s",
                     epsg.c_str());
        }
    }
    ...
    if (epsg.empty() && stacDiscovery.IsValid())
    {
        ...
        const CPLJSONObject &stacExtensions =
            stacDiscovery.GetObj("stac_extensions");
            ...
            if (geomCrs.IsValid())
            {
                ...
                if (geomProps.IsValid())
                {
                    ...
                    if (nGeomEpsg != 0)
                    {
                        ...
                        CPLDebug("EOPFZARR",
                                 "Found CRS code in STAC geometry: %s",
                                 epsg.c_str());
                    }
                }
            }
    ...
    if (epsg.empty())
    {
        ...
        if (!tileName.empty() && tileName.length() >= 3 && tileName[0] == 'T')
        {
            ...
            if (zone >= 1 && zone <= 60)
            {
                ...
                CPLDebug("EOPFZARR",
                         "Inferred EPSG %s from Sentinel-2 tile %s (zone %d, "
                         "%s hemisphere)",
                         epsg.c_str(), tileName.c_str(), zone,
                         isNorth ? "North" : "South");
            }
        }
    }
    ...
    if (!epsg.empty())
    {
        ...
        ds.SetMetadataItem("proj:epsg",
                           epsg.c_str());  // Keep this for STAC compatibility
        CPLDebug("EOPFZARR", "Set EPSG metadata: %s", epsg.c_str());

        // Check if we have a UTM projection (EPSG codes 32601-32660 and 32701-32760)
        isUTM = (nEPSG >= 32601 && nEPSG <= 32660) ||
                (nEPSG >= 32701 && nEPSG <= 32760);
    ...
    if (!wkt.empty())
    {
        ...
        CPLDebug("EOPFZARR",
                 "Set spatial_ref metadata and projection from WKT: %s",
                 wkt.c_str());
    }
    else if (nEPSG != 0)  // Use nEPSG which is an int
    {
        if (oSRS.importFromEPSG(nEPSG) == OGRERR_NONE)
        {
            ...
            CPLFree(pszExportWKT);
            CPLDebug("EOPFZARR",
                     "Set spatial_ref metadata and projection from EPSG: %d",
                     nEPSG);
        }
        else
        {
            CPLDebug("EOPFZARR",
                     "Failed to import EPSG:%d, falling back to WGS84", nEPSG);
            goto set_wgs84;
        }
    }
    else
    ...
    set_wgs84:
        ...
        oSRS.SetAuthority("GEOGCS", "EPSG",
                          4326);  // Ensure EPSG authority for WGS84
        ...
        CPLFree(pszExportWKT);
        CPLDebug("EOPFZARR",
                 "Set default WGS84 spatial_ref metadata and projection");
    ...
    {  // Assuming this is the target EPSG for proj:bbox
        ...
        if (stacDiscovery.IsValid())
        {
            ...
            if (properties.IsValid())
            {
                ...
                if (projBbox.IsValid() && projBbox.Size() >= 4)
                {
                    ...
                    foundProjBbox = true;
                    CPLDebug("EOPFZARR",
                             "Found proj:bbox in STAC properties: "
                             "[%.2f,%.2f,%.2f,%.2f]",
        ...
        if (foundProjBbox)
        {
            ...
            if (width > 0 && height > 0)
            {
                ...
                ds.SetMetadataItem("geo_transform",
                                   transformStr.c_str());  // Keep for metadata
                CPLDebug("EOPFZARR",
                ...
                ds.SetMetadataItem(
                    "utm_easting_min",
                    CPLString().Printf("%.8f", bboxMinX).c_str());
                ds.SetMetadataItem(
                    "utm_easting_max",
                    CPLString().Printf("%.8f", bboxMaxX).c_str());
                ds.SetMetadataItem(
                    "utm_northing_min",
            ...
            {
                CPLDebug("EOPFZARR",
                         "Cannot set geotransform from proj:bbox due to "
                         "invalid raster dimensions: W=%d, H=%d",
                         width, height);
            }
        }
        else
        {
            CPLDebug(
                "EOPFZARR",
                "proj:bbox not found for EPSG:%d, using default UTM values",
                nEPSG);
            ...
            ds.SetMetadataItem("utm_easting_max",
                               "515360.00000000");  // Example based on 512*30m
            ds.SetMetadataItem("utm_northing_min",
                               "4984640.00000000");  // Example based on 512*30m
        ...
        if (geoRefPoints.IsValid())
        {
            ...
            if (ul.IsValid() && lr.IsValid())
            {
                ...
                if (minX != 0.0 || maxX != 0.0 || minY != 0.0 || maxY != 0.0)
                {
                    hasBounds = true;
                    CPLDebug("EOPFZARR",
                             "Found bounds in 'geo_ref_points': "
                             "[%.8f,%.8f,%.8f,%.8f]",
                             minX, minY, maxX, maxY);
                }
            }
    ...
    if (!hasBounds && stacDiscovery.IsValid())
    {
        ...
        if (properties.IsValid())
        {
            CPLJSONArray geoBbox =
                properties.GetArray("bbox");  // STAC bbox (lon,lat,lon,lat)
            if (geoBbox.IsValid() && geoBbox.Size() >= 4)
            {
                ...
                hasBounds = true;
                isUTM =
                    false;  // Explicitly not UTM if we are using STAC geographic bbox
                CPLDebug("EOPFZARR",
    ...
    if (!hasBounds)
    {
        ...
        CPLDebug("EOPFZARR", "Creating default bounds: nEPSG=%d, isUTM=%s",
                 nEPSG, isUTM ? "true" : "false");
        ...
        if (nEPSG != 0 && isUTM)
        {
            ...
            CPLDebug(
                "EOPFZARR",
                "No specific bounds found, using default geographic bounds but "
                "preserving UTM projection (EPSG:%d): [%.8f,%.8f,%.8f,%.8f]",
        ...
        else
        {
            ...
            CPLDebug("EOPFZARR",
                     "No specific bounds found, created default geographic "
                     "bounds (EPSG:4326): [%.8f,%.8f,%.8f,%.8f]",
                     minX, minY, maxX, maxY);
        }
    }
    ...
    if (hasBounds && isUTM &&
        nEPSG !=
            32632)  // Only if UTM and not already handled by EPSG 32632 block
    {
        ...
        bool boundsLookGeographic =
            (std::abs(minX) <= 180 && std::abs(maxX) <= 180 &&
        ...
        if (boundsLookGeographic)
        {
            ...
            if (tgtSRS_transform.importFromEPSG(nEPSG) == OGRERR_NONE)
            {
                OGRCoordinateTransformation *poCT =
                    OGRCreateCoordinateTransformation(&srcSRS_transform,
                                                      &tgtSRS_transform);
                if (poCT)
                {
                    double utmMinX = minX,
                           utmMinY = minY;  // Temp vars for transformation
                    ...
                    {
                        CPLDebug("EOPFZARR",
                                 "Transformed geographic-like bounds "
                                 "[%.8f,%.8f,%.8f,%.8f] to UTM (EPSG:%d) "
                                 "[%.8f,%.8f,%.8f,%.8f]",
                                 minX, minY, maxX, maxY, nEPSG, utmMinX,
                                 utmMinY, utmMaxX, utmMaxY);
                        // Update bounds to UTM coordinates
                        minX =
                            std::min(utmMinX, utmMaxX);  // Ensure minX is min
                        minY =
                            std::min(utmMinY, utmMaxY);  // Ensure minY is min
                        maxX =
                            std::max(utmMinX, utmMaxX);  // Ensure maxX is max
                        maxY =
                            std::max(utmMinY, utmMaxY);  // Ensure maxY is max
                    }
                    else
                    {
                        CPLDebug("EOPFZARR",
                                 "Failed to transform geographic-like bounds "
                                 "to UTM (EPSG:%d)",
                                 nEPSG);
                    }
            ...
            {
                CPLDebug("EOPFZARR",
                         "Failed to import target UTM SRS for transformation "
                         "(EPSG:%d)",
                         nEPSG);
            }
        }
        ...
        {
            CPLDebug("EOPFZARR",
                     "Bounds do not look geographic, skipping UTM "
                     "transformation for EPSG:%d",
                     nEPSG);
        }
    }
    ...
    if (hasBounds)
    {
        ...
        const char *minYName =
            isUTM ? "utm_northing_min" : "geospatial_lat_min";
        const char *maxYName =
            isUTM ? "utm_northing_max" : "geospatial_lat_max";
        ...
        {  // If geographic, ensure these are also set
            ds.SetMetadataItem("geospatial_lon_min",
                               CPLString().Printf("%.8f", minX).c_str());
            ds.SetMetadataItem("geospatial_lon_max",
                               CPLString().Printf("%.8f", maxX).c_str());
            ds.SetMetadataItem("geospatial_lat_min",
                               CPLString().Printf("%.8f", minY).c_str());
            ds.SetMetadataItem("geospatial_lat_max",
                               CPLString().Printf("%.8f", maxY).c_str());
        }
        ...
        {
            CPLDebug("EOPFZARR",
                     "Cannot set general geotransform due to invalid raster "
                     "dimensions: W=%d, H=%d",
                     width, height);
        }
    }
    ...
    {
        CPLDebug("EOPFZARR",
                 "No valid bounds found to calculate geotransform.");
    }
}
                                  ...
                                  const std::string &metadataFile)
{
    ...
    else
    {
        ...
        return CPLFormFilenameSafe(basePath.c_str(), metadataFile.c_str(),
                                   nullptr);
    }
}
}
...
static bool LoadZMetadata(const std::string &rootPath, CPLJSONDocument &doc)
{
    ...
    {
        CPLDebug("EOPFZARR",
                 "'metadata' object does not contain '.zattrs' object.");
        return false;
    }
                               ...
                               const CPLJSONObject &metadata)
{
    ...
    GDALDataset *poZarrDS =
        (GDALDataset *)GDALOpen(zarrPath.c_str(), GA_ReadOnly);
    if (!poZarrDS)
    {
        CPLDebug("EOPFZARR", "Failed to open Zarr dataset: %s",
                 zarrPath.c_str());
    ...
    {
        CPLDebug("EOPFZARR", "No subdatasets found in Zarr dataset: %s",
                 zarrPath.c_str());
        ds.SetMetadataItem("SUBDATASET_COUNT", "0");
        GDALClose(poZarrDS);
        return;
    ...
    {
        const char *pszName = CSLFetchNameValue(
            papszZarrSubdatasets, CPLSPrintf("SUBDATASET_%d_NAME", i + 1));
        const char *pszDesc = CSLFetchNameValue(
            papszZarrSubdatasets, CPLSPrintf("SUBDATASET_%d_DESC", i + 1));
        if (pszName && pszDesc)
        {
            nActualSubdatasets++;
            ...
            if (STARTS_WITH_CI(pszName, "ZARR:"))
            {
                ...
                if (pszInternalPath)
                {
                    CPLString pathPart(pszName + 5,
                                       pszInternalPath - (pszName + 5));
                    eopfName.Printf("EOPFZARR:%s%s", pathPart.c_str(),
                                    pszInternalPath);
                }
                else
                {
            ...
            CPLDebug("EOPFZARR", "Set %s = %s", nameKey.c_str(),
                     eopfName.c_str());
        }
    }

    ds.SetMetadataItem("SUBDATASET_COUNT",
                       CPLString().Printf("%d", nActualSubdatasets).c_str());
...
void EOPF::AttachMetadata(GDALDataset &ds, const std::string &rootPath)
{
    ...
    {
        CPLDebug("EOPFZARR",
                 "Loaded metadata from .zmetadata (consolidated format)");
        hasMetadata = true;
    }
    else
    {
        // Fall back to .zattrs
        // Fall back to .zattrs using proper path construction
        std::string zattrsPath = ConstructMetadataPath(rootPath, ".zattrs");
        if (doc.Load(zattrsPath))
        {
            CPLDebug("EOPFZARR", "Loaded metadata from .zattrs at %s",
                     zattrsPath.c_str());
            hasMetadata = true;
        }
        else
        {
            CPLDebug("EOPFZARR", "Failed to load .zattrs from %s",
                     zattrsPath.c_str());
        }
    }
    ...
    if (hasMetadata)
    {
        const CPLJSONObject &root =
            doc.GetRoot();  // This is .zattrs content now
        ExtractCoordinateMetadata(root, ds);
    }
    else
    {
        CPLDebug("EOPFZARR",
                 "No .zmetadata or .zattrs found in %s, creating defaults for "
                 "coordinates.",
                 rootPath.c_str());
        CPLJSONObject emptyObj;
        ExtractCoordinateMetadata(emptyObj,
                                  ds);  // Setup default coordinate info
    }
    ...
    if (pszSpatialRef && strlen(pszSpatialRef) > 0)
    {
        ...
        if (oSRS_final.SetFromUserInput(pszSpatialRef) == OGRERR_NONE)
        {
            ...
            if (ds.GetProjectionRef() == nullptr ||
                strlen(ds.GetProjectionRef()) == 0 ||
                !EQUAL(ds.GetProjectionRef(), pszFinalWKT))
            {
                ds.SetProjection(pszFinalWKT);
                CPLDebug("EOPFZARR", "Ensured main dataset projection is set "
                                     "from spatial_ref metadata.");
            }
            CPLFree(pszFinalWKT);
        }
        ...
        if (papszDomainList)
        {
            for (int i = 0; papszDomainList[i] != nullptr; i++)
            {
                ...
                {
                    ds.SetMetadataItem("SRS", pszSpatialRef,
                                       papszDomainList[i]);
                    CPLDebug("EOPFZARR", "Set SRS in domain %s",
                             papszDomainList[i]);
                }
            }
