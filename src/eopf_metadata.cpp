...
static void ExtractCoordinateMetadata(const CPLJSONObject &obj, GDALDataset &ds)
{
    ...
    if (isUTM)
    {
        ...
        auto fetchProjBbox = [&](const CPLJSONObject &node)
        {
            CPLJSONArray arr = node.GetArray("proj:bbox");
            if (arr.IsValid() && arr.Size() >= 4)
            {
                bboxMinX = arr[0].ToDouble();
                bboxMinY = arr[1].ToDouble();
                bboxMaxX = arr[2].ToDouble();
                bboxMaxY = arr[3].ToDouble();
                return true;
            }
            return false;
        };
        ...
        if (stacDiscovery.IsValid())
        {
            const CPLJSONObject &props = stacDiscovery.GetObj("properties");
            foundProjBbox = props.IsValid() && fetchProjBbox(props);
        }
        // 2) top-level fallback
        if (!foundProjBbox)
        {
            foundProjBbox = fetchProjBbox(obj);
        }
        if (foundProjBbox)
        {
            CPLDebug("EOPFZARR",
                     "Using proj:bbox for UTM EPSG:%d → [%.2f,%.2f,%.2f,%.2f]",
                     nEPSG, bboxMinX, bboxMinY, bboxMaxX, bboxMaxY);
            ...
            if (width > 0 && height > 0)
            {
                double gt[6];
                gt[0] = bboxMinX;
                gt[1] = (bboxMaxX - bboxMinX) / width;
                gt[2] = 0.0;
                gt[3] = bboxMaxY;
                gt[4] = 0.0;
                gt[5] = -(bboxMaxY - bboxMinY) / height;
                ...
                for (int i = 0; i < 6; ++i)
                {
                    if (i)
                        gtStr += ",";
                    gtStr += CPLString().Printf("%.12f", gt[i]);
                }
                ds.SetMetadataItem("geo_transform", gtStr.c_str());
            }
            return;
        }
    }
...
static bool LoadZMetadata(const std::string &rootPath, CPLJSONDocument &doc)
{
    ...
    if (IsUrlOrVirtualPath(rootPath))
    {
        if (rootPath.back() == '/')
            zmetaPath = rootPath + ".zmetadata";
        else
            zmetaPath = rootPath + "/.zmetadata";
    }
    else
    {
        // For local files, you can use CPLFormFilename (which is safe for local paths)
        zmetaPath = CPLFormFilename(rootPath.c_str(), ".zmetadata", nullptr);
    }
...
{
    // Extract the actual path if rootPath starts with "ZARR:"
    std::string actualPath = rootPath;
    if (STARTS_WITH_CI(rootPath.c_str(), "ZARR:\""))
    {
        size_t start = 6; // After "ZARR:\""
        size_t end = rootPath.find('\"', start);
        if (end != std::string::npos)
        {
            actualPath = rootPath.substr(start, end - start);
        }
        else
        {
            actualPath = rootPath.substr(5); // Fallback, include "ZARR:" if malformed
        }
    }
    CPLDebug("EOPFZARR", "Extracted actual path for metadata loading: %s", actualPath.c_str());

    CPLJSONDocument doc;
    bool hasMetadata = false;
    std::string zmetaPath, zattrsPath;

    // First try to load from .zmetadata (consolidated)
    if (LoadZMetadata(actualPath, doc))  // Pass actualPath instead of rootPath
    {
    ...
    {
        if (IsUrlOrVirtualPath(actualPath))
        {
            if (actualPath.back() == '/')
                zattrsPath = actualPath + ".zattrs";
            else
                zattrsPath = actualPath + "/.zattrs";
        }
        else
        {
            zattrsPath = CPLFormFilename(actualPath.c_str(), ".zattrs", nullptr);
        }
    ...
    {
        CPLDebug("EOPFZARR", "No .zmetadata or .zattrs found in %s, creating defaults for coordinates.", actualPath.c_str());
