...
{
GDALDataset* DatasetOpener::OpenMainDataset(const std::string& path,
                                            ...
                                            char** originalOptions)
{
    ...
    if (STARTS_WITH_CI(path.c_str(), "/vsi"))
    {
        ...
        EOPFErrorUtils::ErrorHandler::Debug(
            "Detected virtual file system path, formatting for Zarr driver");
    ...
    {
        EOPFErrorUtils::ErrorHandler::Debug(
            std::string("Formatted path failed, trying original path: ") + path);
        poUnderlyingDS =
            static_cast<GDALDataset*>(GDALOpenEx(path.c_str(),
                                                 openFlags | GDAL_OF_RASTER | GDAL_OF_READONLY,
                                                 azDrvList,
                                                 openOptions,
                                                 nullptr));
    ...
    if (!poUnderlyingDS && STARTS_WITH_CI(path.c_str(), "/vsicurl/"))
    {
        ...
        EOPFErrorUtils::ErrorHandler::Debug(std::string("VSI path failed, trying direct URL: ") +
                                            directUrl);

        std::string directZarrPath = "ZARR:\"" + directUrl + "\"";
        poUnderlyingDS =
            static_cast<GDALDataset*>(GDALOpenEx(directZarrPath.c_str(),
                                                 openFlags | GDAL_OF_RASTER | GDAL_OF_READONLY,
                                                 azDrvList,
                                                 openOptions,
                                                 nullptr));
...
GDALDataset* DatasetOpener::OpenSubdataset(const std::string& mainPath,
