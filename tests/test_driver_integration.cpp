  ...
  public:
    static void runAllTests()
    {
        ...
        if (driverPath)
        {
            ...
            if (!loadedDriver)
            {
                std::cout << "Driver not auto-loaded, this is expected for plugin testing"
                          << std::endl;
            }
        }
  ...
  private:
    ...
    static void testDriverIdentification()
    {
        ...
        for (const auto& path : testPaths)
        {
            ...
            GDALDriverH identifiedDriver = GDALIdentifyDriverEx(
                path.c_str(), GDAL_OF_RASTER | GDAL_OF_READONLY, nullptr, nullptr);
            ...
            {
                std::cout << "    Path: " << path
                          << " -> No driver identified (expected for non-existent files)"
                          << std::endl;
            }
        }
    ...
    static void testMetadataRetrieval()
    {
        ...
        // Test opening a dataset (this will likely fail for non-existent URLs, but tests the code
        // path)
        const char* testUrl =
        ...
        {
            std::cout << "    Could not open test dataset (expected for network issues)"
                      << std::endl;
    ...
    static void testErrorHandling()
    {
        ...
        std::vector<std::string> invalidPaths = {
            ...
            "EOPFZARR:\"/vsicurl/https://invalid.url.that.does.not.exist/file.zarr\""  // Invalid
                                                                                       // URL
        };
